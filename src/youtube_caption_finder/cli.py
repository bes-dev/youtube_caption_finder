"""
Command-line interface for youtube_caption_finder.

This module allows the library to be used via the command line.
"""
...
def main():
    # Create an argument parser and define CLI arguments.
    ...
    # Create a client instance and execute the search
    client = YoutubeCaptionFinder()
    try:
        if args.all:
            for video in client.search_all(args.query, channel_id=args.channel, filters=filters, sort_option=sort_option):
                print(video)
        else:
            videos = client.search(args.query, channel_id=args.channel, filters=filters, sort_option=sort_option)
            for video in videos:
                print(video)
